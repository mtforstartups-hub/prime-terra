...
function createTransporter() {
  ...
  if (!host || !user || !pass) {
    throw new Error(
      `SMTP configuration is incomplete. Host: ${host ? "OK" : "MISSING"}, User: ${user ? "OK" : "MISSING"}, Pass: ${pass ? "OK" : "MISSING"}`
    );
  }
...
export async function submitContactForm(
  ...
  try {
    ...
    return {
      status: "error",
      message:
        "We were unable to send your message at this time. Please check your network connection or reach out directly.",
    };
  }
}
